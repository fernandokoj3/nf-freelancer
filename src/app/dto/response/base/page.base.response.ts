...
import { PageableOptions } from '@/dto/request/base/page.base.request';
...
export class PageMetaResponse {
  ...
  constructor(
    ...
    const total = Math.ceil(totalItems / options.limit);

    this.order_by = options.order + ' ' + options.sort;
    this.page = options.page || 1;
    this.limit = total > 1 ? options.limit : totalItems;
    this.total_items = totalItems;
    this.total_pages = total > 1 ? total : 1;
